...
function BarGraph({ percentage, range }) {
  return (
    <div className="flex flex-col items-center gap-2">
      ...
      {percentage >= "30" ? (
        <>
          <div className={`w-17 bg-[#f88d38] rounded-xl`} style={{ height: `${Math.floor(percentage)*4}px` }}></div>
        ...
        <>
          <div className={`w-17 bg-[#f88e3830] rounded-xl`} style={{ height: `${Math.floor(percentage)*4}px` }}></div>
  ...
  return (
    <div className="w-full h-full bg-[#f5f7fba7] flex flex-col gap-1 items-center p-3 rounded-3xl">
      ...
      <div className="w-full h-full rounded-2xl flex flex-col gap-2 justify-center">
        ...
        {graphData.map((item)=>{
          return(
            <BarGraph key={item.id} percentage={item.percentage} range={item.range}/>
          )
        })}
        </div>
      </div>
